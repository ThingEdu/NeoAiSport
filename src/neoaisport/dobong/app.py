"""Đỡ Bóng — vòng lặp chính: webcam + MediaPipe Hands (hoặc chuột) đỡ bóng.

  python -m neoaisport.dobong.app            # camera
  python -m neoaisport.dobong.app --source mouse
"""
...
from neoaisport.dobong.render import VolleyRenderer
...
def run(source="camera", db_path="neoaisport.db", sound=True):
    ...
    pygame.display.set_caption("Đỡ Bóng — NeoAiSport · Dế Foundation")
    clock = pygame.time.Clock()
    lb = Leaderboard(db_path)
    ctrl = VolleyController(leaderboard=lb)
    src = get_source("hand", source)
    renderer = VolleyRenderer(screen, source_name=src.name)
...
def main():
    ap = argparse.ArgumentParser(description="Đỡ Bóng — NeoAiSport")
