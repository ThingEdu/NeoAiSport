"""Hứng Mưa — vòng lặp chính: webcam + MediaPipe Pose (hoặc chuột) hứng giọt mưa.

  python -m neoaisport.huongmua.app            # camera
  python -m neoaisport.huongmua.app --source mouse
"""
...
from neoaisport.huongmua.game import RainController
from neoaisport.huongmua.render import RainRenderer
...
def run(source="camera", db_path="neoaisport.db", sound=True):
    ...
    pygame.display.set_caption("Hứng Mưa — NeoAiSport · Dế Foundation")
    clock = pygame.time.Clock()
    lb = Leaderboard(db_path)
    ctrl = RainController(leaderboard=lb)
    src = get_source("pose", source)
    renderer = RainRenderer(screen, source_name=src.name)
...
def main():
    ap = argparse.ArgumentParser(description="Hứng Mưa — NeoAiSport")
